...
And when we look at this integral, it is an expectation under $$\pi_\theta(\tau)$$. It's some quantity multiplied by $$\pi_\theta(\tau)$$ and we integrate over all possible trajectories. That means we can put the expectation back. 
...
Remember that when take a logarithm of multiplication it turns to summation. Now we need the gradient of this equation. The important thing to note in this huge summation, very few things actually depend on $$\theta$$. So initial state distribution $$\log p(s_1)$$ is not affected by $$\theta$$. It's derivate w.r.t $$\theta$$ is **0**. $$\log p(s_{t+1} \mid s_t, a_t)$$ *the transition probability* is also not affected by $$\theta$$, so it's derivative is also **0**. If we substitute the remaining term: 
